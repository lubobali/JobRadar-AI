...
| 3 | Processing of unstructured data | Job descriptions: HTML → text → contextual chunks → `vector(384)` ([`embeddings.py`](src/jobradar/embeddings.py)), **and** an LLM reading each description against the resume ([`scoring.py`](src/jobradar/scoring.py)) | **60,860 vectors** over 5,540 jobs; **300 LLM-scored**, spanning 8–92, mean 42 |
